...
class RunwayProvider:
    ...
    Supported ratios: 1280:720 (landscape), 720:1280 (portrait)
    ...
    # Official model for text-to-video
    MODEL = "gen4.5"

    # Official version header
    API_VERSION = "2024-11-06"

    # Default parameters for Gen-4.5 text-to-video
    DEFAULT_RATIO = "1280:720"
    DEFAULT_DURATION = 5

    # Officially documented Gen-4.5 supported ratios
    SUPPORTED_RATIOS = {"1280:720", "720:1280"}

    # Officially documented Gen-4.5 duration range
    MIN_DURATION = 2
    MAX_DURATION = 10
    ...
    def generate_video(
        ...
        """
        Generate video from text prompt (text-to-video only).

        For text-only generation, promptImage is omitted from request.

        Args:
            prompt: Video generation prompt
            ratio: Aspect ratio (1280:720 or 720:1280)
        ...
        Raises:
            VideoGenerationError: If generation request fails
        """
        ...
        if duration < self.MIN_DURATION or duration > self.MAX_DURATION:
            raise VideoGenerationError(
                ...
                f"Gen-4.5 supports {self.MIN_DURATION}-{self.MAX_DURATION}s"
            )
        ...
        try:
            ...
            # Official text-to-video request (NO promptImage)
            payload = {
                "model": self.MODEL,
                "promptText": prompt,
                "ratio": ratio,
            ...
            logger.info(
                f"Runway: Submitting text-to-video request: "
                f"Gen-4.5 {ratio} {duration}s"
            )
            ...
            # Official response contains task id
            if "id" not in data:
                raise VideoGenerationError(
    ...
    def get_task_status(self, task_id: str) -> dict:
        ...
        Returns:
            Dictionary with status and optional output
    ...
    def wait_for_completion(
        ...
        Raises:
            VideoTimeoutError: If polling exceeds timeout
            PollingError: If task fails
        """
        ...
        while True:
            ...
            try:
                ...
                # Official documented status values
                if status_data["status"] == "SUCCEEDED":
                    output = status_data.get("output")
                    if not output or len(output) == 0:
                        raise PollingError(
                            f"Task succeeded but no output returned"
                        )
                    ...
                    raise PollingError(f"Runway task failed")

                elif status_data["status"] == "CANCELED":
                    logger.error(f"Runway: Task {task_id} was canceled")
                    raise PollingError(f"Runway task was canceled")
    ...
    def download_video(self, output_url: str, output_path: str) -> None:
        ...
        Args:
            output_url: URL to download from
    ...
    def validate_video(self, file_path: str) -> dict:
        ...
        try:
            ...
            logger.info(f"Runway: Video validation passed")
            return metadata
        except Exception as e:
