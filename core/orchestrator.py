...
class PipelineOrchestrator:
    ...
    def run(
        ...
        *,
        ...
        """Run incomplete scenes and checkpoint after every successful scene.

        The executor performs actual provider/media work. The orchestrator adds
        deterministic scene decisions, continuity prompts, provider fallback,
        and durable state without storing credentials in project state.
        """
        ...
        for scene in state.scenes:
            ...
            scene.visual_dna_id = self.visual_dna.style
            ...
            try:
                fallback = run_with_fallback(providers, lambda _provider: executor(scene))
                result = fallback.value
                scene.provider = result.provider or fallback.provider
                ...
                scene.visual_dna_id = result.visual_dna_id or self.visual_dna.style
